...
(function() {
  var Highlight, Journo, allPosts, catchErrors, compareManifest, fatal, fs, htmlPath, knox, loadConfig, loadManifest, manifestPath, marked, ncp, path, postName, postPath, postUrl, s3, shared, writeManifest, _,
  ...
  Journo.render = function(source) {
    return catchErrors(function() {
      var content, lexed, markdown, title, _ref;
      shared.layout || (shared.layout = _.template(fs.readFileSync('layout.html').toString()));
      markdown = _.template(source.toString())({
        fs: fs
      });
      lexed = marked.lexer(markdown);
      title = (_ref = _.find(lexed, function(token) {
  ...
  Journo.FTP.publishPost = function(post) {
    return fs.readFile(postPath(post), function(err, content) {
      return shared.ftp.put(new Buffer(Journo.render(content)), post, function(err) {
        if (err) {
          throw err;
        }
      });
  ...
  compareManifest = function(posts) {
    var deletes, entry, file, meta, puts, stat, _i, _len, _ref;
    puts = [];
    deletes = [];
  ...
  Journo.feed = function() {
    var RSS, config, content, description, feed, lexed, post, sorted, title, _i, _len, _ref, _ref1, _ref2;
    RSS = require('rss');
    loadConfig();
    ...
    sorted = _.sortBy(_.keys(shared.manifest), function(post) {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      title = (_ref1 = _.find(lexed, function(token) {
        return token.type === 'heading';
  ...
  allPosts = function() {
    return fs.readdirSync('posts').filter(function(f) {
      return f.charAt(0) !== '.';
    });
  };
