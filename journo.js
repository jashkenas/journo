...
(function() {
  var Highlight, Journo, allPosts, compareManifest, config, emptyManifest, fatal, fs, ftp, knox, loadConfig, loadManifest, manifest, manifestPath, marked, ncp, path, postPath, postUrl, s3, siteUrl, skeleton, writeManifest, _,
  ...
  Journo.build = function() {
    var dir, html, markdown, name, post, _i, _len, _ref, _results;
    loadConfig();
    loadManifest();
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      _results.push(fs.writeFileSync("" + dir + "/index.html", html));
    }
    return _results;
  };
  ...
  Journo.FTP.publishPost = function(post) {
    return fs.readFile(postPath(post), function(err, content) {
      return ftp.put(new Buffer(Journo.render(content)), post, function(err) {
        if (err) {
          throw err;
        }
      });
